...
public class PhotoDetailsSqliteRepository implements PhotoDetailsRepository {
	...
	private Pattern limitClauseDetector;

	public PhotoDetailsSqliteRepository(Pattern limitClauseDetector, SQLiteOpenHelper dbHelper) {
		this.limitClauseDetector = limitClauseDetector;
	...
	public PhotoDetails toPhoto(Cursor cursor) {
		...
		int idx;
		if ((idx = cursor.getColumnIndex(COL_IDENTIFIER)) != -1)
			photo.setIdentifier(cursor.getLong(idx));
		...
		if ((idx = cursor.getColumnIndex(COL_PHOTO_HIGH)) != -1)
			photo.setPhotoHigh(cursor.getString(idx));
		if ((idx = cursor.getColumnIndex(COL_PERMALINK)) != -1)
			photo.setPermalink(cursor.getString(idx));
		if ((idx = cursor.getColumnIndex(COL_PERMALINK_META)) != -1)
			photo.setPermalinkMeta(cursor.getString(idx));
		if ((idx = cursor.getColumnIndex(COL_NOTES_URL)) != -1)
			photo.setNotesUrl(cursor.getString(idx));
		if ((idx = cursor.getColumnIndex(COL_HEIGHT_HIGH_RES)) != -1)
			photo.setHeightHighRes(cursor.getInt(idx));
		if ((idx = cursor.getColumnIndex(COL_WIDTH_HIGH_RES)) != -1)
			photo.setWidthHighRes(cursor.getInt(idx));
	...
	private static String appendLimitClause(Pattern limitDetector, String cmd, int start, int count) {
		if (cmd == null) {
			return null;
		}

		Matcher matcher = limitDetector.matcher(cmd);
		if (matcher.find()) {
			return cmd;
		}
